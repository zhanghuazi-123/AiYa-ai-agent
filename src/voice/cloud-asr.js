...
function createAliyunSession(apiKey, lang, onTranscript, onError, onClose) {
  ...
  ws.on('message', (data) => {
    try {
      ...
      console.log('[Aliyun WS] 收到事件:', event, 'payload:', JSON.stringify(msg?.payload?.output?.sentence || msg?.payload?.output))
...
// ─── 豆包 (火山引擎) V3 ASR ───
import { createDoubaoSession } from './cloud-asr-doubao.js'

// ─── 工厂函数 ───
// config: { provider, lang, aliyunApiKey?, tencentSecretId?, tencentSecretKey?,
//           tencentAppId?, xunfeiAppId?, xunfeiApiKey?, doubaoAppId?, doubaoAccessToken?,
//           doubaoResourceId? }
export function createCloudASRSession(config, onTranscript, onError, onClose) {
  ...
  if (provider === 'doubao') {
    if (!config.doubaoAppId || !config.doubaoAccessToken) {
      onError('未配置豆包 APP ID 或 Access Token'); return null
    }
    return createDoubaoSession({
      appId: config.doubaoAppId,
      accessToken: config.doubaoAccessToken,
      resourceId: config.doubaoResourceId || 'volc.bigasr.sauc.duration',
      lang,
